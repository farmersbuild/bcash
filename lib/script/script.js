...
class Script {
  ...
  execute(stack, flags, tx, index, value, sigchecks) {
    ...
    let checkBits;
    let minimal = false;
    let nativeIntrospection = false;
    let int64Bits = false;

    if (flags & Script.flags.VERIFY_MINIMALDATA)
      minimal = true;

    if (flags & Script.flags.VERIFY_NATIVE_INTROSPECTION)
      nativeIntrospection = true;

    if (flags & Script.flags.VERIFY_INT_64BITS)
       int64bits = true;
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKLOCKTIMEVERIFY: {
          ...
          if (!num.isSafe())
            throw new ScriptError('INVALID_64_BIT_RANGE', op, ip);
