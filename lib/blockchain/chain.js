...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    // Native Introspection is enabled.
    if (mtp >= this.network.block.nativeIntrospectionTime) {
      state.nativeIntrospection = true;
      state.flags |= Script.flags.VERIFY_64_BIT_INTEGERS;
    }
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasNativeIntrospection() && state.hasNativeIntrospection())
      this.logger.warning('Native Introspection has been activated.');
...
class DeploymentState {
  ...
  constructor() {
    ...
    this.asert = false;
    this.axion = false;
    this.nativeIntrospection = false;
  }
  ...
  /**
   * Test whether Native Introspection update is active.
   * @returns {Boolean}
   */

  hasNativeIntrospection() {
    return this.nativeIntrospection;
  }
